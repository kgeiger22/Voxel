...
public static class EditTerrain
{
    ...
    public static void DestroyBlocks(RaycastHit hit, int explosionsize)
    {
        Chunk chunk = hit.collider.GetComponent<Chunk>();
        if (chunk == null)
            return;
        ...
        for (int x = -explosionsize; x <= explosionsize; ++x)
        {
            for (int y = -explosionsize; y <= explosionsize; ++y)
            {
                ...
                {
                    WorldPos blockpos = GetBlockPos(hit.transform.position);
                    Vector3 deltapos = new Vector3(x, y, z);
                    if (deltapos.magnitude < explosionsize + 0.5f)
                    {
                        chunk.world.SetBlock(new Vector3(x + blockpos.x, y + blockpos.y, z + blockpos.z), new BlockAir());
                    }
                }
            }
        }
